...
import { X, AlertCircle, Info, Image as ImageIcon, Users, Plus, Trash2, CheckCircle2, Youtube, Layout, Globe, Star, Tag, Hash } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [secRes, genRes, platRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL}/api/admin/sections`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_API_URL}/api/admin/genres`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_API_URL}/api/admin/platforms`, { headers: { Authorization: `Bearer ${token}` } })
        ])
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-left-4 duration-500">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                  <div className="group">
                    ...
                    >
                      <option value="movie" className="bg-[#121212]">Movie Feature</option>
                      <option value="series" className="bg-[#121212]">Series / Show</option>
                      <option value="documentary" className="bg-[#121212]">Documentary</option>
                      <option value="short" className="bg-[#121212]">Short Film</option>
                    </select>
                  </div>
                  ...
                  <div className="group lg:col-span-2">
                    ...
                    <div className="flex flex-wrap gap-2 p-4 bg-white/[0.02] border border-white/10 rounded-2xl min-h-[58px]">
                      {availableGenres.length === 0 ? (
                        <p className="text-[10px] text-white/20 font-bold uppercase py-2">No genres defined in system</p>
                      ) : (
                        availableGenres.map(g => {
                          const isSelected = formData.genre.split(',').map(tag => tag.trim()).includes(g.name)
                          return (
                            <button
                              key={g.id}
                              type="button"
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                  <div className="group">
                    ...
                    <select
                      ...
                      className="w-full px-6 py-4 bg-white/[0.03] border border-white/10 rounded-2xl text-white font-bold focus:outline-none focus:border-blue-500/50 transition-all appearance-none cursor-pointer shadow-inner"
                    >
                      <option value="" className="bg-[#121212]">Select Section</option>
                      {sections.map(s => (
                        <option key={s.id} value={s.name} className="bg-[#121212]">{s.name}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="lg:col-span-2 p-6 bg-blue-500/5 border border-blue-500/10 rounded-[1.5rem] flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-black text-white text-sm uppercase tracking-widest">Public Rating</h4>
                        <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Aggregate Score (0.0 - 10.0)</p>
                      </div>
                    </div>
                    <input
                      type="number"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    ...
                    <div className="aspect-[2/3] w-full max-w-[200px] mx-auto bg-white/5 rounded-2xl border border-white/10 overflow-hidden shadow-2xl group relative">
                      ...
                      <span className="absolute bottom-4 left-0 right-0 text-center text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Poster Preview</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="aspect-video w-full bg-white/5 rounded-2xl border border-white/10 overflow-hidden shadow-2xl group relative">
                      <img
                        ...
                        onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/1280x720?text=No+Backdrop' }}
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                      <span className="absolute bottom-4 left-0 right-0 text-center text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Backdrop Preview</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="group">
                    ...
                    >
                      ...
                      {availablePlatforms.map(p => (
                        <option key={p.id} value={p.name.toLowerCase()} className="bg-[#121212]">{p.name}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="p-8 bg-amber-500/5 border border-amber-500/10 rounded-[2rem] space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-black text-white tracking-widest uppercase text-sm">Feature Status</h4>
                          <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest">Promote to Hero Section</p>
                        </div>
                      </div>
                    ...
                    {formData.featured && (
                      <div className="animate-in slide-in-from-top-4 duration-300">
                        <label className="block text-[11px] font-black text-white/30 uppercase tracking-[0.2em] mb-3">
                          Featured Loop Video URL (YouTube)
                        </label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-12 pr-6 py-4 bg-black/40 border border-white/10 rounded-2xl text-white text-sm font-bold focus:outline-none focus:border-amber-500/50 transition-all shadow-xl"
                          />
                        </div>
                      </div>
                    )}
            ...
            {activeTab === 'cast' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
                <div className="flex items-center justify-between pb-4 border-b border-white/5">
                  <div>
                    <h3 className="text-lg font-black text-white uppercase tracking-widest">Ensemble Cast</h3>
                    <p className="text-xs text-white/30 font-bold uppercase tracking-widest mt-1">Add and manage production talent</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-1 gap-6">
                  {castMembers.length === 0 ? (
                    <div className="py-20 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-[2rem]">
                      ...
                      <p className="text-white/30 font-bold uppercase tracking-widest text-xs">No cast members assigned yet</p>
                    </div>
                  ) : (
                    castMembers.map((member, idx) => (
                      <div key={idx} className="group p-6 bg-white/[0.03] border border-white/5 rounded-[2rem] hover:border-white/10 hover:bg-white/[0.05] transition-all flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
                        ...
                        <div className="relative flex-shrink-0">
                          <div className="w-24 h-24 rounded-full border-2 border-white/10 overflow-hidden shadow-2xl group-hover:border-blue-500/30 transition-all duration-500 bg-white/5">
                            <img
                              ...
                              onError={(e) => { (e.target as HTMLImageElement).src = `https://api.dicebear.com/7.x/initials/svg?seed=${idx}&backgroundColor=292929` }}
                            />
                          </div>
                        ...
                        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                          <div>
                            <label className="block text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-2">Talent Name</label>
                            <input
                              type="text"
                              value={member.name}
                          ...
                          <div>
                            <label className="block text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-2">Character Role</label>
                            <input
                              type="text"
                              value={member.role}
                          ...
                          <div>
                            <label className="block text-[9px] font-black text-white/30 uppercase tracking-[0.2em] mb-2">Profile Image URL</label>
                            <input
                              type="url"
                              value={member.image}
...
function Loader({ size, className }: { size?: number, className?: string }) {
  return (
    <div className={`animate-spin rounded-full border-2 border-current border-t-transparent ${className}`} style={{ width: size, height: size }} />
  )
}
